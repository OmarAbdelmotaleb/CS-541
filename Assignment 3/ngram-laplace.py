...
# N-Gram LM with Laplace Smoothing
#####################################################################
...
def P(sentence, n, dict_words, history):
    ...
    for i in range(len(words)):
        ...
        kV = len(dict_words.keys())

        # If the words is not in the dictionary, just set the Pr to 1
        if words_s not in dict_words:
            probs.append(1) # Making this 1 / kV resulted in divide by zeros
        # If it is,
        else:
...
plt.title('Part 4 - N-Gram Perplexity with Laplace Smoothing')
